...
const obtenerUsuarioId = async (req, res) => {
  ...
  try {
    ...
    else usuario.fecha_nacimiento = usuario.fecha_nacimiento.toString().split("T")[0];
...
const crearUsuario = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id_usuario: userId, email }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
const eliminarUsuario = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    connection = await conectarBDMySql();
    await connection.execute("UPDATE usuarios SET habilita = 0 WHERE id_usuario = ?", [id]);
...
const google_login = async (req, res) => {
  ...
  try {
    const ticket = await client.verifyIdToken({
      idToken: token,
      audience: "125703789007-m6785nj61t63qvdjkok8qokrd9tsdoog.apps.googleusercontent.com",
    });
