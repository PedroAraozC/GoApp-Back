...
export const verifyTokenBackOffice = (req, res) => {
  res.json({ ok: true });
};

export const crearUsuarioWeb = async (req, res) => {
  let connection;
...
export const editarUsuarioWeb = async (req, res) => {
  let connection;

  try {
    const { id_usuario, id_rol, habilita } = req.body;
    ...
    const [rows] = await connection.execute(
      "SELECT id_rol, habilita FROM usuarios WHERE id_usuario = ?",
      [id_usuario]
    );
    ...
    const rolAnterior = rows[0].id_rol;
    const habilitaAnterior = rows[0].habilita;

    // 2️⃣ Actualizar usuario
    await connection.execute(
      "UPDATE usuarios SET id_rol = ?, habilita = ? WHERE id_usuario = ?",
      [id_rol, habilita, id_usuario]
    );

    // 3️⃣ Verificar registro en conductores
    const [conductorRows] = await connection.execute(
      "SELECT id_conductor FROM conductores WHERE id_usuario = ?",
      [id_usuario]
    );

    const existeConductor = conductorRows.length > 0;

    // 🟢 Caso: ahora es conductor
    if (id_rol === 3) {
      if (!existeConductor) {
        await connection.execute(
          "INSERT INTO conductores (id_usuario, conectado, habilita) VALUES (?, 0, ?)",
          [id_usuario, habilita]
        );
      } else {
        await connection.execute(
          "UPDATE conductores SET habilita = ? WHERE id_usuario = ?",
          [habilita, id_usuario]
        );
      }
    }

    // 🔴 Caso: deja de ser conductor
    if (rolAnterior === 3 && id_rol !== 3 && existeConductor) {
      await connection.execute(
        "UPDATE conductores SET habilita = 0, conectado = 0 WHERE id_usuario = ?",
        [id_usuario]
      );
    }

    // 🔒 Si se deshabilita usuario
    if (habilita === 0 && existeConductor) {
      await connection.execute(
        "UPDATE conductores SET habilita = 0, conectado = 0 WHERE id_usuario = ?",
        [id_usuario]
      );
